    ...
    <motion.div className="space-y-6" ref={containerRef} onMouseMove={handleMouseMove} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        {onBackToList && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-panel hover:bg-panel border border-white/10 text-xs font-bold text-ink-muted/80 transition cursor-pointer shadow-xs"
          >
        ...
        <div className="flex items-center gap-2.5 ml-auto">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-2xl bg-panel hover:bg-panel border border-white/10 text-xs font-bold text-ink-muted transition cursor-pointer shadow-xs"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-accent-cyan text-primary hover:bg-accent-cyan/80 text-ink text-xs font-extrabold transition cursor-pointer shadow-sm shadow-accent-cyan/20"
          >
      ...
      <div className="glass-panel glow-edge-cyan rounded-3xl overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs font-bold px-2.5 py-0.5 rounded-lg bg-white/5 text-ink-muted border border-white/10 font-mono">
                  DIGITAL ID: {passport.id}
                </span>
                <span className="text-xs font-bold px-2.5 py-0.5 rounded-lg bg-accent-teal/20 text-accent-teal border border-accent-teal/30 flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  Verified Authentic
                </span>
              </div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-ink tracking-tight font-display mt-1">
                {passport.title}
              </h1>
            </div>
          </div>
        ...
        <div className="px-6 sm:px-8 py-4 bg-panel border-b border-white/10">
          <p className="text-xs font-bold text-ink-muted/80 uppercase tracking-wider mb-2.5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {simpleSteps.map((step) => (
              ...
              >
                ...
                <p className="text-[11px] text-ink-muted/80">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {qrCodeUrl && (
              <div className="bg-panel p-4 rounded-2xl border border-white/10 flex items-center gap-3.5">
                ...
                <div className="text-xs">
                  ...
                  <button
                    onClick={() => setShowQrModal(true)}
                    className="text-xs text-accent-teal hover:text-[#00C853] mt-1 cursor-pointer font-bold flex items-center gap-1"
                  >
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-panel p-4 rounded-2xl border border-white/10">
                <span className="text-[11px] text-ink-muted/80 font-bold uppercase block">Cleanliness Score</span>
                <p className="text-2xl font-extrabold text-accent-teal font-mono mt-1">{passport.reusabilityScore}%</p>
                <p className="text-[11px] text-ink-muted mt-0.5">Top-grade clean</p>
              </div>

              <div className="bg-panel p-4 rounded-2xl border border-white/10">
                <span className="text-[11px] text-ink-muted/80 font-bold uppercase block">Total Weight</span>
                <p className="text-2xl font-extrabold text-ink font-mono mt-1">{passport.quantityMT} Tons</p>
                <p className="text-[11px] text-ink-muted mt-0.5">{passport.quantityMT * 1000} Kilograms</p>
              </div>

              <div className="bg-panel p-4 rounded-2xl border border-white/10">
                <span className="text-[11px] text-ink-muted/80 font-bold uppercase block flex items-center gap-1">
                ...
                <p className="text-[11px] text-ink-muted mt-0.5">Tons of CO₂ gas</p>
              </div>
              ...
              <div className="bg-panel p-4 rounded-2xl border border-white/10">
                ...
                <p className="text-[11px] text-ink-muted mt-0.5">₹{passport.valuation.basePricePerMT.toLocaleString("en-IN")}/Ton</p>
              </div>
            </div>
            ...
            <div className="bg-white/5 p-5 rounded-2xl border border-white/10 space-y-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div className="bg-transparent p-3 rounded-xl border border-white/10">
                  <span className="text-[10px] text-ink-muted uppercase font-bold block">Current Owner</span>
                  <p className="font-extrabold text-ink mt-0.5 truncate">{passport.ownerOrg}</p>
                  <p className="text-[10px] text-ink-muted/80 font-mono mt-0.5">GST: {passport.ownerGstin}</p>
                </div>

                <div className="bg-transparent p-3 rounded-xl border border-white/10">
                  <span className="text-[10px] text-ink-muted uppercase font-bold block">Factory City & State</span>
                  <p className="font-extrabold text-ink mt-0.5 truncate">{passport.locationCity}</p>
                  <p className="text-[10px] text-ink-muted/80 mt-0.5">{passport.locationState}, India</p>
                </div>

                <div className="bg-transparent p-3 rounded-xl border border-white/10">
                  <span className="text-[10px] text-ink-muted uppercase font-bold block">Local Authority</span>
                  <p className="font-extrabold text-copper/400 mt-0.5 truncate">{passport.spcbJurisdiction}</p>
                  <p className="text-[10px] text-accent-teal font-bold mt-0.5">✓ Green Safety Certified</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {passport.suggestedApplications.map((app, i) => (
                  <div key={i} className="text-xs text-ink flex items-center gap-2 bg-accent-teal/10 p-3 rounded-xl border border-accent-teal/30 font-medium">
            ...
            {/* Simple Green Fact */}
            <div className="bg-accent-teal/10 p-4 rounded-2xl border border-accent-teal/30 text-xs text-ink space-y-1">
            ...
            <div className="pt-2">
              ...
              {showAdvancedTech && (
                <div className="mt-2 p-4 bg-white/5 rounded-2xl border border-white/10 space-y-2 text-xs font-mono animate-fadeIn">
                  <div>
                    <span className="text-[10px] text-ink-muted font-bold uppercase">SHA-256 Unique Proof Hash</span>
                    <div className="flex items-center justify-between bg-transparent p-2 rounded-lg border border-white/10 mt-1">
                      <span className="text-[11px] text-ink-muted/80 truncate">{passport.recordHash}</span>
                  ...
                  {passport.ledgerTxHash && (
                    <div>
                      <span className="text-[10px] text-ink-muted font-bold uppercase">Polygon Ledger Transaction ID</span>
                      <p className="text-[11px] text-ink-muted/80 bg-transparent p-2 rounded-lg border border-white/10 mt-1 truncate">
                        {passport.ledgerTxHash}
                      </p>
                    </div>
                  )}
          ...
          {events.length > 0 ? (
            events.map((ev) => (
              <div key={ev.id} className="relative">
                ...
                <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-extrabold text-ink">{ev.eventType.replace(/_/g, " ")}</span>
                    <span className="text-[11px] text-ink-muted/80 font-medium">
            ...
            <div className="relative">
              ...
              <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-xs">
                ...
                <p className="text-[11px] text-ink-muted mt-1">Logged by: {passport.ownerOrg}</p>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="glass-panel glow-edge-cyan rounded-3xl p-6 sm:p-7 max-w-lg w-full space-y-5 shadow-2xl animate-fadeIn my-6">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              ...
              <button
                ...
                className="text-ink-muted/80 hover:text-ink-muted/80 text-xs cursor-pointer font-bold px-2 py-1 rounded-lg hover:bg-slate-100 transition"
              >
                ✕ Close
              </button>
            </div>
            ...
            <div className="bg-panel p-4 rounded-3xl border border-white/10 text-center">
              ...
              <p className="text-[10px] text-ink-muted font-mono mt-2 font-semibold">
                SCAN WITH ANY CAMERA • DIRECT AADHAAR TEXT PAYLOAD
              </p>
            </div>

            {/* Digital Aadhaar Data Summary Card */}
            <div className="bg-panel p-4 rounded-2xl border border-white/10 space-y-2 text-xs">
            ...
            <div className="grid grid-cols-2 gap-2.5 pt-1">
              <button
                ...
                className="py-2.5 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 border border-white/10 text-xs font-bold text-ink-muted transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                ...
                className="py-2.5 px-3 rounded-xl bg-accent-cyan text-primary hover:bg-accent-cyan/80 text-ink text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-sm shadow-accent-cyan/20"
              >
            ...
            <p className="text-[10px] text-center text-ink-muted/80 font-mono">
