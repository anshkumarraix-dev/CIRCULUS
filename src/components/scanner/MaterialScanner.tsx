...
export function getCategoryBadgeProps(cat: string | undefined): { label: string; bg: string; text: string; border: string; icon: string } {
  ...
  if (c.includes("ferrous") || c.includes("steel") || c.includes("iron")) {
    return { label: "Ferrous Metal (Steel/Iron)", bg: "bg-blue-500/20", text: "text-blue-300", border: "border-blue-500/40", icon: "🔩" };
  }
            ...
            <div>
              ...
              {analysisResult ? (
                <div className="space-y-4 mt-4 animate-fadeIn text-xs">
                  ...
                  <div className="bg-blue-500/5 p-4 rounded-2xl border border-blue-500/20 space-y-1.5">
                    <p className="font-extrabold text-xs text-blue-400 flex items-center gap-1.5">
                    ...
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-1">
                      {analysisResult.suggestedApplications.map((app, i) => (
                        <li key={i} className="text-xs text-slate-300 flex items-center gap-1.5 bg-[#12181F] p-2.5 rounded-xl border border-blue-500/20 font-medium">
                  ...
                  <div className="bg-gradient-to-br from-slate-900 to-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-800 text-white space-y-3 shadow-md">
                    <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-800">
                      ...
                      >
                        <RefreshCw className={`w-3.5 h-3.5 ${isSearchGroundingLoading ? "animate-spin text-blue-400" : "text-slate-400"}`} />
                    ...
                    {isSearchGroundingLoading ? (
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {searchGrounding.sources.map((src, i) => (
                                <a
                                  ...
                                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-blue-300 hover:text-blue-200 text-[11px] font-medium border border-slate-700 transition"
                                >
                      ...
                      <div className="text-center py-3">
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs inline-flex items-center gap-1.5 transition cursor-pointer"
                        >
                ...
                <div className="py-16 text-center space-y-3">
                  <div className="w-14 h-14 rounded-3xl bg-blue-500/10 text-blue-400 flex items-center justify-center mx-auto shadow-xs border border-blue-500/20">
