  ...
  return (
    <div className="bg-panel p-3.5 rounded-2xl border border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto overflow-hidden">
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-copper/10 text-copper border border-copper/30 font-body font-mono text-xs font-semibold uppercase tracking-wider shrink-0">
          <span className="w-2 h-2 rounded-full bg-copper animate-pulse"></span>
          REAL-TIME FEED
        </div>

        {recentEvents.length > 0 ? (
          <div className="flex items-center gap-2 overflow-hidden text-sm truncate">
            ...
            <span className="text-sm font-mono text-silver/60 font-body shrink-0">
              ({recentEvents[0].location.split(",")[0]})
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-2 text-sm text-silver/60 font-body">
      ...
      <button
        ...
        className="w-full sm:w-auto px-4 py-2 rounded-xl bg-copper hover:bg-copper/90 text-ink font-body font-semibold text-sm uppercase tracking-tight flex items-center justify-center gap-1.5 transition cursor-pointer shadow-sm shadow-copper/20 shrink-0"
      >
